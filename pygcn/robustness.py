...
# This class mostly serves as an automatic run and storage for the bounds. All methods
    ...
    def __init__(self, model, x, adj, eps, targets=None):
        ...
        beta_u.append(bu)
        beta_l.append(bl)
        lb, ub, Lambda, Omega, J_tilde, lmd, omg, delta, theta = self.compute_bound_and_variables(weights, adj, x, eps, alpha_u, alpha_l, beta_u, beta_l)
        LB.append(lb)
        UB.append(ub)
    ...
    def compute_first_layer_preac_bounds(x, eps, weights, adj, targets):
        # return GCNBoundsRelaxed.compute_first_layer_preac_bounds(x, eps, weights, adj, targets)
        return GCNBoundsFull.compute_first_layer_preac_bounds(x, eps, weights, adj, targets).view(x.shape[0], -1)
    ...
    def compute_bound_and_variables(weights, adj, x, eps, alpha_u, alpha_l, beta_u, beta_l):
        ...
        # TODO: Vectorize
        for j in range(J):
            lmd_l[:, :, j] = (alpha_u[ind] * (weights[-1][:, j] > 0).float().repeat(N, 1) +
                                alpha_l[ind] * (weights[-1][:, j] <= 0).float().repeat(N, 1))
            omg_l[:, :, j] = (alpha_l[ind] * (weights[-1][:, j] > 0).float().repeat(N, 1) +
                                alpha_u[ind] * (weights[-1][:, j] <= 0).float().repeat(N, 1))
            delta_l[:, :, j] = (beta_u[ind] * (weights[-1][:, j] > 0).float().repeat(N, 1) +
                                beta_l[ind] * (weights[-1][:, j] <= 0).float().repeat(N, 1))
            theta_l[:, :, j] = (beta_l[ind] * (weights[-1][:, j] > 0).float().repeat(N, 1) +
                                beta_u[ind] * (weights[-1][:, j] <= 0).float().repeat(N, 1))
        # Lambda and Omega
        # Lambda_l = w.mm(lmb_tilde_l * Lambda[0])
        # Omega_l = w.mm(omg_tilde_l * Omega[0])
        ...
        return t1_l + t2_l + t3_l, t1_u + t2_u + t3_u, Lambda, Omega, J_tilde, lmd, omg, delta, theta


# Special full bounds for two layers only. Specifically avoids using kronecker products
# to improve memory usage.
# TODO: Work in progress, do not use yet.
class GCNBoundsTwoLayerTensor():
    # This class mostly serves as an automatic run and storage for the bounds. All methods
    # are static and can be used independently of a class instance.
    # Assumes the adjacency matrix has been normalized and had the identity added.
    # These are the full bounds in the paper using the Kronecker product. Note that this
    ...
    def compute_first_layer_preac_bounds(x, eps, weights, adj, targets):
        ...
        LB_first, UB_first = torch.zeros(NJ), torch.zeros(NJ)
        for j in range(NJ):
            w_j = tensor_product(adj, weights[0][j])
            Ax0_j = xo.mm(w).view(-1)
            if targets is None:
                dualnorm_Aj = torch.sum(torch.abs(w[:, j]))  # dual of inf, 1 norm
            else:
                dualnorm_Aj = torch.sum(torch.abs(w[targs, j]))  # dual of inf, 1 norm
